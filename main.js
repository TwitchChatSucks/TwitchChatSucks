var channels = ['GamesDoneQuick']//['alca', 'twitchplayspokemon', 'europeanspeedsterassembly', 'sc2starleague', 'IAmSp00n', 'cnotbusch', 'Utorak007', 'WolfsGoRawr'], // Channels to initially join
...
var chat = document.getElementById('chat'), chatSpam = document.getElementById('chat-spam'),
...
function messageValue(text, emotes){
	var textChars = text.split('');
	var textWords = text.split(' ');
	var numAllChars = textChars.length;
	var numEmoteChars = 0;
	var numEmotes = 0;
	var numCapslock = 0;
	var numNonValuableChars = 0; //Chars like spaces
	var numDoubledWordsChars = 0;
	if(emotes != undefined || emotes != null){
	for(var i in emotes){
		numEmoteChars -= (eval((emotes[i].toString())) + 1);
		numEmotes++;
	}
	}
	var lastWasCaps = false;
	for(var c in textChars){
		var char = textChars[c];
	...
	var lastWord = "";
	for(var w in textWords){
		var word = textWords[w];
		if(word == lastWord){
			numDoubledWordsChars += word.length;
		}else{
			lastWord = word;
		}
	}
	var ret = numAllChars;
	ret -= (numEmoteChars + numEmotes + numDoubledWordsChars + numNonValuableChars);
	//ret -= numCapslock;
	//console.log("Filter: " + numAllChars + " " + numEmoteChars + " " + numEmotes);
	console.log("Filter: "+text+ " " + numAllChars+" -> " + numEmoteChars);
	return ret;
}
...
function handleChat(channel, user, message, self) {
	...
	if(allChatMessages.indexOf(message)==-1 
		&& messageValue(message, user.emotes) > 10){
		allChatMessages.push(message);
		chat.appendChild(chatLine);
	}else{
		chatSpam.appendChild(chatLine);
	}
